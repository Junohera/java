...
public class Server03 {
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         s = sS.accept();
         Sender sender = new Sender(s); // 송신을 담당하는 역할의 클래스(쓰레드 기능상속됨)
         Receiver reveiver = new Receiver(s); // 수신을 담당하는 역할의 클래스(쓰레드 기능 상속됨)
         sender.start(); // 각각 독립적으로 스타트
         reveiver.start(); // 각각 독립적으로 스타트
      } catch (IOException e) { System.out.println("서버 소켓 생성에 실패했습니다."); }
