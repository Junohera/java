...
public class Collection01 {
	public static void main(String[] args) {
		// 동적 배열의 객체 생성 초기값으로 크기를 지정할 수 있지만 통상 크기 지정없이 사용함
		Vector v = new Vector();
		ArrayList a = new ArrayList();
		
		// 데이터 입력 add 메서드를 사용하여 데이터를 입력
		v.add(10);
		v.add(20);
		v.add(30);
		...
		// Vector, ArrayList 클래스의 size 메서드 : 현재 저장되어 있는 데이터의 개수를 반환
		...
		a.set(2, 100); // 인덱스 매개변수를 추가하여 데이터 삽입 가능 - set은 기존값을 지우고 대입
		System.out.println();
		for (int i = 0; i < a.size(); i++)
			System.out.printf("a[%d] = %d\t\t", i, a.get(i));
