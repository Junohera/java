...
public class Method03 {
	/*
	 * 매개변수 : method 호출시 method가 필요로하는 데이터를 전달해줄 수 있는 매개역할의 변수
	 * 매개 변수의 선언
	 * public static void method이름(자료형 매개변수, 자료형 매개변수, ...) {
	 * } 매개 변수 또한 필요한 갯수 만큼 선언해서 사용할 수 있습니다.
	 */
	
	// 아래 method는 method 실행을 위해 정수형 값하나를 매개변수로 전달 받는 형태로 선언
	public static void myPrint(int n) {
		System.out.println("전달받은 매개변수의 값 n -> " + n);
	}
	
	// 아래 method는 다수의 매개변수를 전달 받는 method의 형태 - 각 매개변수 별로 전달받을 변수를 선언합니다.
