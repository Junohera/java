 ...
 * 반면, 멤버 메서드는 생성된(오버로딩 포함) 메서드별로 하나만 존재하며 모든 객체들이 공유하게 됩니다.
 * 이때 현재 메서드를 호출한 객체가 어떤 객체인지 구분하여 실행될 수 있게 this라는 숨겨진 참조변수를 제공합니다.
 */
class ThisA {
	private int num;
	 ...
	 * 매개변수로 선언된 num 변수는 현재 메서드안에서만 사용되는 지역 변수로만 사용되고 소멸합니다.
	 * 멤버 메서드에서는 멤버변수에 접근 권한이 있으므로, 참조변수 this와 변수가 (.)으로 연결되어 사용됩니다.
	 */
	public void display() {
	...
	// this의 용도 1 : 멤버메서드를 호출한 호출객체의 참조변수값(주소값)을 전달받아 사용한다.
...
public class Class18 {
	public static void main(String[] args) {
		ThisA a1 = new ThisA();
		ThisA a2 = new ThisA();
		a1.setNum(10); // 멤버변수에 전달 인수 값을 저장하기 위한 메서드 호출
		a2.setNum(20);
		a1.display();
		a2.display();
