...
/*
 *  static 키워드
 *  클래스, 멤버변수, 멤버method에 적용할 수 있는 키워드 입니다.
 *  정적 변수(멤버변수), 정적 method를 선언할 때 사용합니다.
 */

/*
 * 자바프로그램의 구동 과정
 * 1. JVM에 의해서 실행할 모든 클래스 중, staticmethod, static멤버필드를 수집해서 메모리에 로딩
 * 프로그램의 시작mainmethod의 실행)전에 메모리 적재
 * 2. 메모리에 로딩된 staticmethod 중, main 이름을 검색
 * 3. mainmethod가 검색되었다면 JVM 해당 mainmethod를 호출하여 프로그램을 시작
 * 
 *  static 필드/method의 특징
 *  프로그램의 구동 전부터 메모리를 확보하고 있는 멤버필드 및 method !!!
 */
class StaticA {
 ...
 * 동적 변수는 main() method 실행전에 자동으로 미리 생성되는 변수입니다.
 */
public class Class21 {
	public static void main(String[] args) {
		// main method 실행전에 스태틱 변수는 이미 생성되있습니다.
		...
		StaticA.number = 10;
		System.out.println(StaticA.number);
		// private으로 보호된 static 변수는 static method를 사용하여 값을 저장하거나 얻어냅니다.
