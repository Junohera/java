...
/*
 * 싱글턴 패턴을 구현하는 단계 1 : 생성자(Constructor) method를 private으로 보호합니다.
 ...
 * 	싱글턴 패턴을 구현하는 단계 3 : 생성해놓은 객체의 참조변수값을 리턴하는 public형의 method를 생성합니다.
 * 											 method 이름 getInstance() - static으로 생성합니다.
 */
class SingletonEx {
	int n1;
	int n2;
	...
	/*
	 * 생성자가 private으로 보호되어 mainmethod 안에서 new SingletonEx()로 새로운 객체 생성이 되지않습니다.
	 * SingletonEx s1 = new SingletonEx(); 에러
	 * 싱글턴 방식은 클래스내부에 유일한 객체(ist)를 생성해두고 (private static으로 생성)
	 * public static으로 만들어진 getInstance() method를 호출하고 ist를 리턴받아 객체를 사용합니다
	 * 생성자 method는 static 인스턴스 ist만들때 최초 실행된 때 말고는 실행되지 않습니다.
	 */
}
public class Class28 {
	public static void main(String[] args) {
		 ...
		 * 반드시 getInstancemethod를 사용하여 객체를 반환받아야 합니다.
		 */
	}
}
